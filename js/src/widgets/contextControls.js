(function ($) {
  ...
  $.ContextControls.prototype = {
    ...
    annotationTemplate: $.Handlebars.compile([
      '{{#if showEdit}}',
      '<a class="mirador-osd-pointer-mode hud-control selected" title="{{t "pointerTooltip"}}"  uk-tooltip="{{t pointerTooltip}}; pos: bottom-left" aria-label="{{t "pointerTooltip"}}" tabindex="0">',
      '<i class="fa fa-mouse-pointer"></i>',
      '</a>',
      '{{#each tools}}',
      '<a class="mirador-osd-{{this.logoClass}}-mode hud-control mirador-osd-edit-mode" title="{{t this.tooltip}}" uk-tooltip="{{t this.tooltip}}; pos: bottom-left" aria-label="{{t this.tooltip}}" tabindex="0">',
      '{{#if this.useFontAwesome}}',
      '<i class="fas {{this.logoClass}}"></i>',
      '{{else}}',
      '<i class="material-icons">{{this.logoClass}}</i>',
      '{{/if}}',
      '<span class="sr-only">{{t this.tooltip}}</span>',
      '</a>',
      '{{/each}}',
      '{{#if showStrokeStyle}}',
